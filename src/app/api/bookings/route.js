...
function parseDateTimeText(value) {
  ...
  if (Number.isNaN(date.getTime())) return null;
  const timeSegment = timePart.split("-")[0]?.trim();
  return { date: formatDateKey(date), time: timeSegment || timePart.trim() };
}
...
export async function GET(req) {
  ...
  try {
    ...
    for (const booking of bookings) {
      ...
      const isIsoDate = typeof dateKey === "string" && /^\d{4}-\d{2}-\d{2}$/.test(dateKey);
      if (dateKey && !isIsoDate) {
        const parsedDate = new Date(String(dateKey));
        dateKey = Number.isNaN(parsedDate.getTime()) ? "" : formatDateKey(parsedDate);
      }
    ...
    const blocked = await db
      ...
      .find({ date: { $gte: fromKey, $lte: toKey } })
    ...
    for (const block of blocked) {
      ...
      if (!block?.date || !Number.isFinite(start) || !Number.isFinite(end)) {
        continue;
      }
      events.push({ date: block.date, start, end });
    }
...
export async function POST(req) {
  ...
  let status = "pending";
  if (paymentIntentId) {
    status = (await isPaymentActive(paymentIntentId)) ? "confirmed" : "pending";
  }
  const doc = {
  ...
  try {
    ...
    if (db && booking && status === "confirmed") {
      const block = computeBlockWindow(booking?.schedule, booking?.service?.packageName);
      if (block) {
        try {
          ...
          console.error("[bookings] failed to upsert booking block:", blockErr);
        }
      }
    }
