...
export function renderCalendar() {
    ...
    for (let i = 1; i <= totalDays; i++) {
      ...
      // Keep the day selectable even if all times are blocked.
...
export function nextMonth() {
  ...
  renderCalendar();
  const from = `${calendar.year}-${pad(calendar.month + 1)}-01`;
  const to = `${calendar.year}-${pad(calendar.month + 1)}-${pad(
    daysInMonth(calendar.year, calendar.month)
  )}`;
  void loadCalendarEvents(from, to);
}
  ...
  export function initDateClicking() {
    $$(".calendar-day").forEach((day) => {
      day.addEventListener("click", () => {
        ...
        void loadCalendarEvents(date, date).then(() => {
          if (calendar.selectedDate !== date) return;
          if (calendar.selectedTime) return;
          renderTimeSlots();
          updateStep3ServiceSummary();
        });
      });
...
export async function loadCalendarEvents(fromOverride?: string, toOverride?: string): Promise<void> {
  try {
    const today = new Date();
    const from = fromOverride || formatDateKey(today);
    const to = toOverride || formatDateKey(addDays(today, EVENT_LOOKAHEAD_DAYS));
    ...
    if (Array.isArray(data?.events)) {
      ...
      const seen = new Set(calendar.events.map((evt) => `${evt.date}-${evt.start}-${evt.end}`));
      incoming.forEach((evt) => {
        const key = `${evt.date}-${evt.start}-${evt.end}`;
        if (!seen.has(key)) {
          seen.add(key);
          calendar.events.push(evt);
        }
      });
      const prevDate = calendar.selectedDate;
      const prevTime = calendar.selectedTime;
      renderCalendar();
      if (calendar.selectedDate) {
        renderTimeSlots();
        if (prevDate && prevTime && prevDate === calendar.selectedDate) {
          const slots = $$(".time-slot");
          const match = slots.find(
            (slot) =>
              slot.textContent?.trim() === prevTime &&
              !slot.classList.contains("disabled")
          );
          if (match) {
            calendar.selectedTime = prevTime;
            $("#selectedTime")?.setAttribute("value", prevTime);
            slots.forEach((s) => s.classList.remove("is-selected", "selected"));
